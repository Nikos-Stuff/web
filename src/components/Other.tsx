...
import TeamCard from "@components/OtherCard.tsx";

type Props = {
    tags: string[]
    data: CollectionEntry<"otherprojects">[]
}

export default function Other({ data, tags }: Props) {
    const [filter, setFilter] = createSignal(new Set<string>())
    const [mps, smps] = createSignal<CollectionEntry<"otherprojects">[]>([])

    createEffect(() => {
        smps(data.filter((entry) =>
            ...
            <div class="col-span-3 sm:col-span-2">
                <div class="flex flex-col">
                    <div class="text-sm uppercase mb-2">
                        SHOWING {mps().length} OF {data.length} STUFF
                    </div>
